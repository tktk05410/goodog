...
def get_recent_trends():
    ...
    date_map = {d: {'new_users': 0, 'new_products': 0, 'new_transactions': 0} for d in date_list}
    ...
    user_results = db.session.query(
        ...
        func.date(User.create_time) >= start_date
    ).group_by(
        func.date(User.create_time)
    ).all()
    for r in user_results:
        d = str(r.date)
        if d in date_map:
            date_map[d]['new_users'] = r.count

    # 新商品
    product_results = db.session.query(
        func.date(Product.create_time).label('date'),
        func.count(Product.id).label('count')
    ).filter(
        func.date(Product.create_time) >= start_date
    ).group_by(
        func.date(Product.create_time)
    ).all()
    for r in product_results:
        d = str(r.date)
        if d in date_map:
            date_map[d]['new_products'] = r.count
    ...
    tx_results = db.session.query(
        ...
        func.date(Transaction.create_time) >= start_date
    ).group_by(
        func.date(Transaction.create_time)
    ).all()
    for r in tx_results:
        d = str(r.date)
        if d in date_map:
            date_map[d]['new_transactions'] = r.count

    data = []
    for d in date_list:
        data.append({
            'date': d,
            'new_users': date_map[d]['new_users'],
            'new_products': date_map[d]['new_products'],
            'new_transactions': date_map[d]['new_transactions']
        })
